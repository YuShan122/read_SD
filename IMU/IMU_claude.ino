...
LiquidCrystal_I2C lcd(0x27,16,2); //inSTANCE
...
void setup() {
  ...
  uint8_t cols = 16;
  uint8_t rows = 2;
  lcd.begin(cols, rows, LCD_5x8DOTS);                                                         //Initialize the LCD
...
void loop(){
  ...
  // Updated gyro angle calculations
  float dt = 0.0000611; // time step: 1 / (250Hz / 65.5)
  
  // Calculate pitch
  float pitch_rate = gyro_x + sin(angle_roll) * tan(angle_pitch) * gyro_y + cos(angle_roll) * tan(angle_pitch) * gyro_z;
  angle_pitch += pitch_rate * dt;
  
  // Calculate roll
  float roll_rate = cos(angle_roll) * gyro_y - sin(angle_roll) * gyro_z;
  angle_roll += roll_rate * dt;
  
  // Calculate yaw
  float yaw_rate = sin(angle_roll) / cos(angle_pitch) * gyro_y + cos(angle_roll) / cos(angle_pitch) * gyro_z;
  angle_yaw += yaw_rate * dt;
